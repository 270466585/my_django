# coding:utf-8
from Public_methods.ConfigParser_tools import Config_tools
from Public_methods import Custom_exception
from Public_methods.file_path import crg_path
import pymysql

class MysqlUtil():
    '''
    mysql数据库相关操作
    连接数据库信息：mysql_info
    创建游标：mysql_execute
    查询某个字段对应的字符串：mysql_getstring
    查询一组数据：mysql_getrows
    关闭mysql连接：mysql_close
    '''
    def __init__(self):
        '''创建连接池方式'''
        self.conn = MysqlUtil.__getConnect()
        self.cur = self.conn.cursor() #创建游标


    @staticmethod
    def __getConnect():
        crg_info = Config_tools(crg_path())
        mysql_info = crg_info.crg_itme('DataBase')
        '''静态方法，从连接池中取出连接'''
        try:
            conn = pymysql.connect(host=mysql_info['host'],
                                   port=int(mysql_info['port']),
                                   user=mysql_info['user'],
                                   passwd=mysql_info['pwd'],
                                   db=mysql_info['db'],)
            return conn
        except Exception as e:
            raise Custom_exception.DatabaseError(Reason=e)

    def mysql_execute(self, sql):
        '''执行sql语句'''
        try:
            self.cur.execute(sql)
        except Exception as a:
            self.conn.rollback()         # sql执行异常后回滚
            print("执行SQL语句出现异常：%s"%a)
        else:
            self.cur.close()
            self.conn.commit()          # sql无异常时提交

    def mysql_getVerify(self, Phone_number):
        ''' 查看手机号对应的验证码'''
        try:
            sql='SELECT code FROM app_verify WHERE user_mobile=%s'%Phone_number
            self.cur.execute(sql)
        except Exception as a:
            print("执行SQL语句出现异常：%s"%a)
        else:
            rows = self.cur.fetchall()
            all_verily = []
            for i in rows:
                 all_verily.append(i)
            self.cur.close()
            return all_verily[-1][0]

    def mysql_getToken(self, Phone_number):
        ''' 查询手机号对应的token和devices_info'''
        try:
            sql='SELECT * FROM app_users WHERE user_mobile=%s'%Phone_number
            self.cur.execute(sql)
            user_id = self.cur.fetchall()
        except Exception as a:
            print("执行SQL语句出现异常：%s"%a)
        else:
            sql='SELECT * FROM app_user_token WHERE user_id=%s'%user_id[0][0]
            self.cur.execute(sql)
            user_token=self.cur.fetchall()
            self.cur.close()
            return user_id[0][0],user_token[0][2],user_token[0][3]

    def mysql_close(self):
        ''' 关闭 close mysql'''
        try:
            self.conn.close()
        except Exception as a:
            print("数据库关闭时异常：%s"%a)

    def query_datas(self,query):
        def wrapper():
            query()
            self.mysql_close()
        return wrapper








# MySQLdb.connect()     建立数据库连接
# cur = conn.cursor()    #通过获取到的数据库连接conn下的cursor()方法来创建游标。
# cur.execute()    #过游标cur 操作execute()方法可以写入纯sql语句。通过execute()方法中写如sql语句来对数据进行操作。
# cur.close()     # cur.close() 关闭游标
# conn.commit()   # conn.commit()方法在提交事物，在向数据库插入(或update)一条数据时必须要有这个方法，否则数据不会被真正的插入。
# conn.rollback() # 发生错误时候回滚
# conn.close()     # Conn.close()关闭数据库连接

if __name__ == "__main__":
    A = MysqlUtil()
    Phone_number='18123317091'
    #print (A.mysql_getVerify(Phone_number)) #获取验证码
    data=A.mysql_getToken(Phone_number)
    print(data[2]) #获取user-token和devices-info


